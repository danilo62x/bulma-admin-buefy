<template>
  <div>
    ...
    <div class="columns is-multiline" style="margin-bottom: 0.5rem;">
      <div v-for="stat in chartStats" :key="stat.key" class="column is-3-desktop is-6-tablet is-12-mobile">
        <div class="tx-stats-card">
          ...
          <div class="tx-stats-info">
            ...
            <div class="tx-stats-trend" :class="stat.trendUp ? 'has-text-success' : 'has-text-danger'">
              <span class="mdi" :class="stat.trendUp ? 'mdi-trending-up' : 'mdi-trending-down'" />
              {{ stat.trend }}
            </div>
          </div>
            ...
            <svg width="60" height="30" viewBox="0 0 60 30">
              <defs>
                <linearGradient :id="`sg-${stat.key}`" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" :stop-color="stat.color" stop-opacity="0.45" />
                  <stop offset="100%" :stop-color="stat.color" stop-opacity="0" />
                </linearGradient>
              </defs>
              <path :d="stat.sparkArea" :fill="`url(#sg-${stat.key})`" />
              <path :d="stat.sparkLine" fill="none" :stroke="stat.color" stroke-width="1.5"
    ...
    <div class="columns">
      <div class="column is-8">
        <CardComponent title="Receita Mensal 2025" icon="mdi-chart-bar">
          <template #toolbar>
            <div style="display:flex;align-items:center;gap:0.75rem;">
              <div class="tx-legend-row">
                <span class="tx-legend-dot" style="background:#485fc7" />
                <span>R$ (mil)</span>
              </div>
              <b-dropdown aria-role="list" position="is-bottom-left">
                ...
                <b-dropdown-item aria-role="listitem">Exportar PNG</b-dropdown-item>
                <b-dropdown-item aria-role="listitem">Exportar CSV</b-dropdown-item>
              </b-dropdown>
            </div>
          </template>
          <div class="tx-chart-wrap">
            <svg width="100%" viewBox="0 0 700 260" class="tx-chart-svg">
              <g v-for="g in barChart.grid" :key="g.y">
                <line :x1="barChart.pl" :y1="g.y" :x2="690" :y2="g.y"
                  stroke="var(--tx-border)" stroke-width="1" stroke-dasharray="4 4" />
                <text :x="barChart.pl - 6" :y="g.y + 4" text-anchor="end"
                  font-size="9.5" fill="var(--tx-text-muted)">{{ g.label }}</text>
              </g>
              <g v-for="(b, i) in barChart.bars" :key="i">
                <rect :x="b.x" :y="b.y" :width="b.w" :height="b.h"
                  fill="#485fc7" rx="4" ry="4"
                  :opacity="barHover === -1 || barHover === i ? 0.85 : 0.3"
                  class="tx-bar-rect"
                  @mouseenter="barHover = i" @mouseleave="barHover = -1"
                />
                <text v-if="barHover === i" :x="b.x + b.w / 2" :y="b.y - 6"
                  text-anchor="middle" font-size="10" font-weight="600" fill="#485fc7">
                  {{ b.value }}k
                </text>
                <text :x="b.x + b.w / 2" :y="255" text-anchor="middle"
          ...
          <div class="tx-donut-wrap">
            <svg width="180" height="180" viewBox="0 0 180 180">
              <circle cx="90" cy="90" r="65" fill="none"
                stroke="var(--tx-border)" stroke-width="24" />
              <circle v-for="seg in donut.segments" :key="seg.label"
                ...
                stroke-linecap="butt"
              />
              <text x="90" y="86" text-anchor="middle" font-size="20" font-weight="700"
                fill="var(--tx-text-heading)">{{ donut.total }}k</text>
              <text x="90" y="103" text-anchor="middle" font-size="10"
                fill="var(--tx-text-muted)">Visitantes</text>
            </svg>
            <div class="tx-donut-legend">
              <div v-for="seg in donut.segments" :key="seg.label" class="tx-donut-leg-item">
                <span class="tx-legend-dot" :style="{ background: seg.color }" />
    ...
    <!-- Area Chart -->
    <CardComponent title="Usuários Ativos — Últimas 24 Semanas" icon="mdi-chart-areaspline">
      <template #toolbar>
        <div class="tx-tab-switcher">
          <button v-for="t in ['Novos', 'Recorrentes', 'Total']" :key="t"
            class="tx-tab-btn" :class="{ 'is-active': areaTab === t }"
            @click="areaTab = t">{{ t }}</button>
        </div>
      </template>
      <div class="tx-chart-wrap">
        <svg width="100%" :viewBox="`0 0 ${area.W} ${area.H}`" class="tx-chart-svg">
          <defs>
          ...
          <g v-for="g in area.grid" :key="g.y">
            <line :x1="area.pl" :y1="g.y" :x2="area.W - 10" :y2="g.y"
              stroke="var(--tx-border)" stroke-width="1" stroke-dasharray="4 4" />
            <text :x="area.pl - 6" :y="g.y + 4" text-anchor="end"
              font-size="9" fill="var(--tx-text-muted)">{{ g.label }}</text>
          </g>
          <text v-for="lbl in area.xLabels" :key="lbl.x"
            :x="lbl.x" :y="area.H - 4" text-anchor="middle"
            font-size="9" fill="var(--tx-text-muted)">{{ lbl.text }}</text>
          <path :d="area.area2" fill="url(#ag2)" />
          <path :d="area.area1" fill="url(#ag1)" />
          <path :d="area.line2" fill="none" stroke="#48c774" stroke-width="2"
            stroke-linecap="round" stroke-linejoin="round" />
          <path :d="area.line1" fill="none" stroke="#485fc7" stroke-width="2.5"
            stroke-linecap="round" stroke-linejoin="round" />
        </svg>
      </div>
      <div class="tx-area-legend">
        ...
        <span class="tx-legend-dot" style="background:#48c774" />
    ...
    <div class="columns" style="margin-top:1rem;">
      <div class="column is-7">
        <CardComponent title="Produtos Mais Vendidos" icon="mdi-package-variant">
          <div>
            <div v-for="(p, i) in topProducts" :key="p.name" class="tx-top-row">
              <div class="tx-top-rank">{{ i + 1 }}</div>
              <div class="tx-top-icon" :style="{ background: `color-mix(in srgb, ${p.color} 13%, transparent)`, color: p.color }">
                <span class="mdi" :class="p.icon" />
              </div>
              <div class="tx-top-info">
                <div class="tx-top-name">{{ p.name }}</div>
                <div class="tx-bar-track">
                  <div class="tx-bar-fill" :style="{ width: p.pct + '%', background: p.color }" />
                </div>
              </div>
      ...
      <div class="column is-5">
        <CardComponent title="Funil de Conversão" icon="mdi-filter-variant">
          <div class="tx-funnel">
            <div v-for="stage in funnel" :key="stage.label" class="tx-funnel-step">
              ...
              <div class="tx-funnel-track">
                <div class="tx-funnel-fill" :style="{ width: stage.pct + '%', background: stage.color }" />
              </div>
              <div class="tx-funnel-rate" v-if="stage.rate">↳ {{ stage.rate }} conversão</div>
            </div>
          </div>
        </CardComponent>
      </div>
...
const ui = useUiStore()
onMounted(() => ui.setPageTitle('Gráficos & Analytics'))

const barHover = ref(-1)
const areaTab = ref('Novos')
...
function spark(vals: number[]) {
  ...
  let line = pts.map((pt, i) => `${i === 0 ? 'M' : 'L'}${pt.x},${pt.y}`).join(' ')
  return {
    line,
